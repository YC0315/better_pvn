...
from lib.datasets.augmentation import crop_or_padding_to_fixed_size, rotate_instance, crop_resize_instance_v1, bright
import random
import torch
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index_tuple):
        ...
        if self.split == 'train':
            ...
            inp = img
            #inp = self.augment1(img)   #测试时降低亮度

        if self._transforms is not None:
            inp, kpt_2d, mask = self._transforms(inp, kpt_2d, mask)

        vertex = pvnet_data_utils.compute_vertex(mask, kpt_2d).transpose(2, 0, 1)
        ret = {'inp': inp, 'mask': mask.astype(np.uint8), 'vertex': vertex, 'img_id': img_id, 'meta': {}, 'kpt_2d':kpt_2d}   # 自己将kpt_2d添加到列表里
