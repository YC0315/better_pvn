...
class Evaluator:
    ...
    def uncertainty_pnp(self, kpt_3d, kpt_2d, var, K):
        ...
        for vi in range(var.shape[0]):
            if var[vi, 0, 0] < 1e-6 or np.sum(np.isnan(var)[vi]) > 0:
                ...
                if np.linalg.det(scipy.linalg.sqrtm(var[vi])) != 0:
    ...
    def evaluate(self, output, batch):
        '''
        # 测试时，可视化查看网络分割的mask和对应真实mask之间的差别
        output1 = output['mask'][0].detach().cpu().numpy()
        batch1 = batch['mask'][0].detach().cpu().numpy() 
